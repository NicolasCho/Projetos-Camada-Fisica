...
def main():
    try:
        ...
        while pckg_no <= n_of_packages:
            package, crc = create_pckg(txBuffer, pckg_no, n_of_packages)
            ...
            if pckg_no == 6 and ERRO_DE_PACOTE:             ####### Para simular erro de ordem de pacote, mudar variável ERRO_DE_PACOTE para True. Numera o pacote 6 como 5  ###############
                package, crc = create_pckg(txBuffer, pckg_no, n_of_packages, error=True)

            com1.sendData(package)

            log += write_line(3, 'envio', len(package), pckg_n = pckg_no, n_of_pckgs=n_of_packages, crc=crc)
