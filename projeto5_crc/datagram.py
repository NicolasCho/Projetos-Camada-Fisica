...
def dg_head(msg_type, n_of_pckgs = 0, pckg_n = 0, archive_id = 0, payload_size = 0, expected_pckg = 0, last_pckg = 0, crc = 0):
    """
    ...
    h8h9 = crc.to_bytes(2, byteorder = 'big')           # CRC
...
def create_pckg(message, pckg_no, n_of_pckgs, error = False):
    ...
    crc = crc_calculator.calculate_checksum(payload) 
        
    head = dg_head(msg_type=3, n_of_pckgs=n_of_pckgs, pckg_n=pckg_no, payload_size=payload_size, crc = crc)
